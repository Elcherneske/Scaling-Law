...
def three_stage_lrs(warmup: int, total: int, lr_a: float, lr_b: float, lr_c: float, stage_a: int, stage_b: int, const_warmup: bool) -> np.ndarray:
    """
    Generate a three-stage learning rate schedule with optional warmup.
    ...
    Args:
        ...
        lr_a (float): Learning rate for the first stage.
        lr_b (float): Learning rate for the second stage.
        lr_c (float): Learning rate for the third stage.
        stage_a (int): Number of steps in the first stage (including warmup).
        stage_b (int): Number of steps in the second stage (including stage_a).
        const_warmup (bool): If True, use constant lr_a during warmup; otherwise, linear ramp-up.
    ...
    warmup_lrs = np.linspace(0, lr_a, warmup) if not const_warmup else np.full(warmup, lr_a)
    stage_a_lrs = np.full(stage_a - warmup, lr_a)
    stage_b_lrs = np.full(stage_b - stage_a, lr_b)
    stage_c_lrs = np.full(total - stage_b, lr_c)
    return np.concatenate((warmup_lrs, stage_a_lrs, stage_b_lrs, stage_c_lrs))
