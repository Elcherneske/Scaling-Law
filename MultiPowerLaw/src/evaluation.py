...
def evaluate_mpl(data: dict, curve_set: list, best_params: list, fig_folder: str):
    ...
    for file_name in curve_set:
        ...
        # Save curve data to TSV file
        curve_data = np.column_stack((step, pred, loss))
        np.savetxt(f"{fig_folder}/{file_id}_mplfit.tsv", curve_data, 
                  delimiter='\t', header='step\tpred\tloss', comments='')
